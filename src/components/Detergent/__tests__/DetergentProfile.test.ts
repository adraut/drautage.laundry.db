...
describe('DetergentProfile', () => {
  ...
  describe('context rules and category exclusions', () => {
    ...
    describe.each(Object.values(DetergentType))('C8_18FattyAcidsSodiumSalt in a %s detergent', (type) => {
      ...
      ])('excludes from Soap when %s (foaming surfactant) precedes it with non-surfactant gap', (surfactant) => {
        const profile = new DetergentProfile(
          'Test',
          'Brand',
          type,
      ...
      it('keeps as Soap when only a non-foaming surfactant precedes with non-surfactant gap', () => {
        const profile = new DetergentProfile(
          'Test',
          'Brand',
          type,
      ...
      it('keeps as Soap when immediately preceded by a surfactant', () => {
        const profile = new DetergentProfile(
          'Test',
          'Brand',
          type,
      ...
      it('keeps as Soap when no surfactant precedes it', () => {
        const profile = new DetergentProfile(
          'Test',
          'Brand',
          type,
    ...
    ])('free-fatty-acid soap rule for %s', (fattyAcid) => {
